...
export function MediaImage({
  ...
  if (!src)
    return (
      <div
        className={cn("media-awaiting", className)}
        role="img"
        aria-label="Project imagery selected after scoping"
      >
        <span>YOUR SITE. YOUR OBJECTIVE.</span>
      </div>
    );
  ...
  return (
    <img
      {...props}
      src={media?.variants.find((v) => v.width >= 1200)?.src ?? src}
      alt={alt === "" ? "" : (media?.alt ?? alt ?? "")}
      width={media?.width ?? props.width}
      height={media?.height ?? props.height}
      loading={loading}
      decoding="async"
      sizes={media ? sizes : undefined}
      srcSet={media?.variants.map((v) => `${v.src} ${v.width}w`).join(", ")}
      className={className}
    />
  );
}
