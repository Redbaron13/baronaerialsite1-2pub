    ...
    <section className={cn("py-20 md:py-28", light ? "bg-paper" : "bg-ink")}>
      <div className="site-container">
        <Reveal>
          ...
          <h2
            className={cn(
              "mt-3 max-w-[24ch] text-[clamp(1.9rem,4vw,3.1rem)]",
          ...
          <p className={cn("lead mt-4 max-w-[62ch]", light ? "text-ink-muted" : "text-fg-soft")}>
            ...
            Airspace Class, LAANC, Part 107 Waivers, SGI Waivers, TFRs, National Security Events,
            and local permits line up. Open a stage. The 3D study uses a dated FAA UAS Facility Map
            snapshot around Newark Liberty. Current restrictions and an actual authorization still
        ...
        >
          <div>
            ...
            <h3 className={cn("mt-2 text-2xl", light ? "text-ink-text" : "text-fg")}>
              {active.title}
            </h3>
            <p
              className={cn(
        ...
        <Reveal delay={80}>
          <p className={cn("mt-8 max-w-[62ch] text-sm", light ? "text-paper-muted" : "text-muted")}>
            A LAANC approval, Part 107 Waiver, or SGI Waivers is planning status — not a contract
            and not a launch. The 3D chart is a teaching schematic of how Class B, Class D, and UAS
            Facility Map cells stack around EWR; operational altitudes come from the current FAA map
            at briefing.{" "}
...
function SgiPanel({ light }: { light: boolean }) {
  return (
    <div className="grid gap-5 sm:grid-cols-2">
      ...
      <p className={cn("text-xs sm:col-span-2", light ? "text-paper-muted" : "text-muted")}>
        {sgiFacts.note}
      </p>
    </div>
  );
...
function LaunchClock({ light }: { light: boolean }) {
  ...
  const tone = scene.result === "GO" ? "go" : scene.result === "HOLD" ? "hold" : "nogo";
  ...
  return (
    <div className={cn("launch-clock", tone)}>
      ...
      <p className="launch-status mt-2 font-display text-4xl font-bold tracking-[0.12em]">
        {scene.result}
      </p>
      <ol className="mt-5 grid gap-2">
        {(
          ...
          >
            ...
            <span
              className={cn(
                "font-display text-xs font-semibold tracking-[0.16em]",
                gateClass(status),
              )}
            >
              {status}
      ...
      <p className={cn("mt-4 text-sm leading-relaxed", light ? "text-ink-muted" : "text-fg-soft")}>
        {scene.note}
      </p>
