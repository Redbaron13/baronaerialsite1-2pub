...
export const Route = createFileRoute("/work/$slug")({
  ...
  loader: ({ params }) => {
    const aliases: Record<string,string> = { "jobsite-cut": "bancroft-listing", "envelope-close": "water-tower", "rail-station": "north-munn-bridge", "twilight-neighborhood": "north-munn-bridge", "warehouse-sunset": "north-munn-bridge", "night-lots": "north-munn-bridge", "recreation-field": "north-munn-bridge", "city-dusk": "city-context", "night-plaza": "night-highway", "east-orange-cityscape": "city-context" };
    if (aliases[params.slug]) throw redirect({ to: "/work/$slug", params: { slug: aliases[params.slug] }, statusCode: 301 });
    const item = work.find((w) => w.slug === params.slug);
    if (!item) throw notFound();
    return item;
  },
  head: ({ loaderData }) => seo({ title: `${loaderData?.title ?? "Work"} — Baron Aerial Media`, description: loaderData?.summary ?? "", path: `/work/${loaderData?.slug ?? ""}`, image: loaderData?.image, type: "article" }),
});
  ...
  return (
    <PageShell tone="light">
      <section className="photo-hero relative isolate min-h-[70vh] overflow-hidden bg-ink">
        <MediaImage src={item.image} alt={item.summary} loading="eager" fetchPriority="high" sizes="100vw" className="absolute inset-0 size-full object-cover" />
          ...
          <section className="border-y border-paper-line bg-ink py-16 md:py-20">
            <div className="site-container">
              ...
              <p className="mt-4 max-w-[62ch] text-fg-soft">
                ...
                what nadir, crosshatch, and oblique actually do. Paths illustrate capture strategies; they are not a replay of flight telemetry.
              </p>
              <div className="mt-8">
          ...
          <section className="border-b border-paper-line bg-ink py-16 md:py-20">
            <div className="site-container grid items-center gap-10 md:grid-cols-2">
              <div>
                ...
                <h2 className="mt-3 text-[clamp(1.7rem,3vw,2.4rem)] text-fg">One capture. Separate deliverables.</h2>
                <p className="mt-4 max-w-[52ch] text-fg-soft">
                  Explore the orthomosaic and elevation visualization as separate layers. The color DEM is shown as a reference image; accurate terrain geometry requires a calibrated elevation export.
                </p>
              </div>
          ...
          <section className="border-b border-paper-line bg-ink py-16 md:py-20">
            <div className="site-container grid items-center gap-10 md:grid-cols-2">
              ...
              <div>
                ...
                <p className="mt-4 max-w-[52ch] text-fg-soft">
                  After capture, each frame is aligned to a 3D model. Capture locations and the colored coverage layer describe the source imagery. A mapping packet can include this diagram
                  with the orthomosaic so a GC or planner can see how the model was built. This still
                  is a reconstruction report from Baron Aerial Media processing — the coverage visualization accompanying this site’s mapping packet.
                </p>
              </div>
            </div>
          </section>
      ...
      {item.film && <section className="site-container pb-16"><h2 className="mb-6 text-2xl">Field film</h2><FieldClip src={item.film} poster={item.slug === "north-munn-bridge" ? "/media/bridge-film-poster.webp" : item.image} alt={item.title} /></section>}
