...
try {
 for(let n=0;n<25;n++) {
  ...
  try {
   ...
   const res=await fetch('https://api.resend.com/emails',{method:'POST',headers:{Authorization:`Bearer ${process.env.RESEND_API_KEY}`,'Content-Type':'application/json','Idempotency-Key':`mission-${item.id}`},body:JSON.stringify({from:process.env.MISSION_BRIEF_FROM,to:[process.env.MISSION_BRIEF_TO],subject:'New Baron Aerial Media mission brief',text:`A new brief is stored securely. Reference: ${item.id}. Review it through your authorized database access.`}),signal:AbortSignal.timeout(10000)});
   if(!res.ok)throw new Error(`Provider HTTP ${res.status}`);
   await pool.query('UPDATE mission_briefs SET delivered_at=now(),delivery_lease_until=NULL,last_error=NULL WHERE id=$1',[item.id]);
   console.log(`[brief-outbox] Delivered notification ${item.id}`);
  } catch(error) {
   await pool.query("UPDATE mission_briefs SET delivery_lease_until=NULL,last_error=$2,next_attempt_at=now()+($3 * interval '1 minute') WHERE id=$1",[item.id,String(error.message).slice(0,100),Math.min(1440,2**Math.min(item.attempts,10))]);
