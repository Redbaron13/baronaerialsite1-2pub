...
for (const [src, meta] of Object.entries(catalog)) {
  ...
  for (const widthTarget of [400, 800, 1200, 1800].filter((w) => w <= width)) {
    ...
    const info = await sharp(input)
      .resize({ width: widthTarget, withoutEnlargement: true })
      .webp({ quality: 82 })
      .toFile("public" + out);
  ...
  smallBytes += variants[0]?.bytes ?? input.length;
}
await fs.writeFile("src/data/assets.manifest.json", JSON.stringify(manifest, null, 2) + "\n");
console.log(
  JSON.stringify({ assets: Object.keys(manifest.assets).length, originalBytes, smallBytes }),
);
